...
import (
	...
	"github.com/williiam/dcard-backend-assignment/internal/app/dao"
...
var (
	...
	urls = map[string]dao.URLMapping{
		shortURL1: {
...
func (f FakeURLRepo) Get(shortURL string) (dao.URLMapping, error) {
	if f.shouldFail {
		return dao.URLMapping{}, errors.New("repo failure")
	}

	return f.urls[shortURL], nil
}

func (f FakeURLRepo) Save(shortURL string, urlMapping dao.URLMapping, expTime time.Duration) error {
	if f.shouldFail {
...
func (f FakeURLRepo) Update(shortURL string, urlMapping dao.URLMapping) error {
	if f.shouldFail {
		return errors.New("repo failure")
	}
